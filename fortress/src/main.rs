...
// 3. Kudu-Energy: Tracking Social Economics & Global Warming Impact
    let solar_array = Registry {
        id: "SOLAR-MACHA-05".to_string(),
        function: KuduFunction::BioLeap("Renewable Energy".to_string()), // Using BioLeap for impact tracking
        location: "Machakos Hub".to_string(),
    };

    let kwh_saved = 1550.5; // Kilowatts generated by solar vs grid
    println!("--- KUDU ENERGY MONITORING ---");
    println!("Device: {} | Energy Tracked: {} kWh | Global Warming Offset: {}kg CO2", 
              solar_array.id, kwh_saved, kwh_saved * 0.7);